...
class AudioProcessor {
  ...
  static Future<void> processWav(String inputPath,
    ...
    for (double pos in chopPositions) {
      int repeatCount = repeatMin + Random().nextInt(repeatMax - repeatMin + 1);
      double segmentDuration = 0.25 + Random().nextDouble() * 0.25;
    ...
    for (double pos in chopPositions) {
      ...
      double segmentDuration = 0.25 + Random().nextDouble() * 0.25;
      filter +=
          "[0:a]atrim=start=${pos.toStringAsFixed(2)}:end=${(pos + segmentDuration).toStringAsFixed(2)},aloop=loop=${repeatCount - 1}:size=2e+09[a$repeatIndex];";
      repeatIndex++;
    }

    String inputs = List.generate(repeatIndex, (i) => "[a$i]").join();
    String finalCmd =
        '-i "$tempScrewed" -filter_complex "$filter $inputs concat=n=$repeatIndex:v=0:a=1[out]" -map "[out]" "$outputPath"';
